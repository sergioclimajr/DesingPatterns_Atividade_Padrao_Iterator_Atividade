...
public class Grafo<T> {
	...
	public List<Vertice<T>> incidentes( Vertice<T> u ){
		 ...
		 for(Aresta<T> arco: arestas){
			if( arco.getDestiny().equals(u))
				vertex.add((Vertice<T>) arco.getOrigin());
			else if( arco.getOrigin().equals(u))
				vertex.add((Vertice<T>) arco.getDestiny());
	...
	public Aresta<T> getAresta( Vertice<T> origem, Vertice<T> destino ){
		...
		for(int i=0 ; i < arestas.size(); i++ ){
			arc = arestas.get(i);
			if ( arc.getOrigin().equals( origem) && arc.getDestiny().equals( destino) )
				return arc;
		}
		return null;
	}
...
//            	r += u.tag + " -> ";
				Vertice<T> v = (Vertice<T>) e.getDestiny();
	...
	public void BFSDistance( T source ){
		...
		while ( !q.isEmpty()){
			...
			for(Aresta arco: u.getAdj() ) {
				@SuppressWarnings("unchecked")
				Vertice<T> w = (Vertice<T>) arco.getDestiny();
	...
	private void runDFS(Vertice<T> u, Grafo<T> grafo){
		...
		for(Aresta<T> arco: uAdjacentes){
			w = (Vertice<T>) arco.getDestiny();
			
			if( w.getStatus() == VertexState.Unvisited ) {							
				runDFS(w,grafo);
				grafo.addAresta(  arco.getOrigin(), arco.getDestiny(), arco.getPeso());								
			}
		} 
	...
	public List<Vertice<T>> getDestinations(Vertice<T> v)
	{
		...
		for( Aresta<T> a : arcos){
			list.add( (Vertice<T>) a.getDestiny() );
	...
	public int getDistance(Vertice<T> start, Vertice<T> end)
	{
		...
		for( Aresta<T> a : arcos){
			if( a.getDestiny().getCarga().equals( end.getCarga()) )
				return a.getPeso();
	...
	public Grafo<T> getSubGrafo(T carga){
		...
		while( !fila.isEmpty() ) {
			...
			for(Aresta edge: element.getAdj()){
				target = grafo.addVertice( (T) edge.getDestiny().getCarga());
				grafo.addAresta(source, target, edge.getPeso());
				fila.add((Vertice<T>) edge.getDestiny());				
			}
